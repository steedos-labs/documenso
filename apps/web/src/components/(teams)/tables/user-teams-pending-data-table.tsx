...
export default function UserTeamsPendingDataTable() {
  ...
  const { mutateAsync: createTeamPendingCheckout } =
    trpc.team.createTeamPendingCheckout.useMutation({
      onSettled: (_data, _error, { teamPendingId }) => removeIdFromMutatingTeamIds(teamPendingId),
      onMutate: ({ teamPendingId }) => addIdToMutatingTeamIds(teamPendingId, 'checkout'),
  ...
  const { mutateAsync: deleteTeamPending } = trpc.team.deleteTeamPending.useMutation({
    onSettled: (_data, _error, { teamPendingId }) => removeIdFromMutatingTeamIds(teamPendingId),
    onMutate: ({ teamPendingId }) => addIdToMutatingTeamIds(teamPendingId, 'delete'),
    onSuccess: () => {
      toast({
        title: 'Success',
  ...
  const addIdToMutatingTeamIds = (teamPendingId: number, type: 'checkout' | 'delete') => {
  ...
  const removeIdFromMutatingTeamIds = (teamPendingId: number) => {
    setMutatingTeamIds((prev) => {
      const ids = { ...prev };
      delete ids[teamPendingId];
      return ids;
    });
  };
          ...
          cell: ({ row }) => (
            <div className="flex justify-end space-x-2">
              <Button
                ...
                onClick={async () => createTeamPendingCheckout({ teamPendingId: row.original.id })}
              >
                Pay
              </Button>
              ...
              <Button
                ...
                onClick={async () => deleteTeamPending({ teamPendingId: row.original.id })}
              >
                Remove
              </Button>
            </div>
